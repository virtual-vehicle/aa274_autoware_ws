...
static std::string global_map_frame_id_ = "map";
...
{
	if (pcd.width != 0) {
		pcd.header.frame_id = global_map_frame_id_; //"map";
		pcd_pub.publish(pcd);
...
//  n.param<std::string>("global_map_frame_id", global_map_frame_id_, "map");
  private_nh.getParam("global_map_frame_id", global_map_frame_id_);
	...
	if (margin < 0) {
	    ...
	    {
	      // load all .pcd files in directory
	      std::string path(argv[2]);
	      std::string file_type = ".pcd";

	      std::vector<std::string> files = globVector(path + "*.pcd");
