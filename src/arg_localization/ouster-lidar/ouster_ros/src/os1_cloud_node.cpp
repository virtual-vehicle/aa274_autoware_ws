...
int main(int argc, char** argv) {
    ...
    int delay_lidar_frame_ms = nh.param<int>("delay_lidar_frame_ms", 0);
    ROS_INFO_STREAM("Delay correction for lidar offset: " << delay_lidar_frame_ms << "ms");
    ...
    auto batch_and_publish = OS1::batch_to_iter<CloudOS1::iterator>(
        ...
        [&](uint64_t scan_ts) mutable {
            ...
            ros::Duration offset_time_lidar = msg.header.stamp - ros::Time::now();
            if ( abs(offset_time_lidar.toSec()) > 0.15 ) {
    	    	//ROS_WARN_STREAM("Lidar provided timestamp: " << msg.header.stamp << " delta_t: " << offset_time_lidar.toSec());
                msg.header.stamp = ros::Time::now();
            }

            if ( abs(delay_lidar_frame_ms) > 0 ) {
                msg.header.stamp = ros::Time::now() + ros::Duration(delay_lidar_frame_ms/1000.0);
            }
    	    //ROS_INFO_STREAM("Lidar timestamp: " << msg.header.stamp);
