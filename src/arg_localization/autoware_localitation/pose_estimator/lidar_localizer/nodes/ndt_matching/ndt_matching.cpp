...
#define MEASUREMENT_MIN_RANGE 0.0
#define MEASUREMENT_MAX_RANGE 200.0

#define Wa 0.4
#define Wb 0.3
#define Wc 0.3

// keep all measurements in 2D space
static bool mod_keep_2D_ = false;
static int process_nth_input_ = 0;
...
static bool enable_filtering_ = true;
static double input_voxel_filter_ = 0.5;

static double min_range_ = MEASUREMENT_MIN_RANGE;
static double max_range_ = MEASUREMENT_MAX_RANGE;
...
static double ndt_res = 1.0;      // Resolution
...
static ros::Publisher diagnostics_pub;
static diagnostic_msgs::DiagnosticArray diagnostics_array_msg;
...
static std::string _output_frame_id = "base_link";
static std::string _input_frame_id = "lidar_top/os_sensor";
...
static void param_callback()
{
 ...
 // if (input->max_iterations != max_iter)
  ...
  if (init_pos_set == 0)
  {
    ...
    localizer_pose.roll = 0.0;
    localizer_pose.pitch = 0.0;
    localizer_pose.yaw = initial_pose.yaw;
    if (mod_keep_2D_ == false)
    {
...
static void imu_odom_calc(ros::Time current_time)
{
  ...
  double diff_imu_roll = 0.0;
  double diff_imu_pitch = 0.0;
  double diff_imu_yaw = imu.angular_velocity.z * diff_time;
  if (mod_keep_2D_ == false)
  {
    diff_imu_roll = imu.angular_velocity.x * diff_time;
    diff_imu_pitch = imu.angular_velocity.y * diff_time;
  }
  ...
  predict_pose_imu_odom.roll = 0.0;
  predict_pose_imu_odom.pitch = 0.0;
...
static void odom_calc(ros::Time current_time)
{
  ...
  double diff_odom_roll = 0.0;
  double diff_odom_pitch = 0.0;
  double diff_odom_yaw = odom.twist.twist.angular.z * diff_time;
  if (mod_keep_2D_ == false)
  {
    diff_odom_roll = odom.twist.twist.angular.x * diff_time;
    diff_odom_pitch = odom.twist.twist.angular.y * diff_time;
  }
...
static void imu_calc(ros::Time current_time)
{
  ...
  predict_pose_imu.roll = 0.0;
  predict_pose_imu.pitch = 0.0;
...
static void imu_callback(const sensor_msgs::Imu::Ptr& input)
{
  ...
  static double previous_imu_yaw = imu_yaw, previous_imu_roll = imu_roll, previous_imu_pitch = imu_pitch;
  double diff_imu_roll = 0.0, diff_imu_pitch = 0.0;
  double diff_imu_yaw = calcDiffForRadian(imu_yaw, previous_imu_yaw);
  if (mod_keep_2D_ == false)
  {
    diff_imu_roll = calcDiffForRadian(imu_roll, previous_imu_roll);
    diff_imu_pitch = calcDiffForRadian(imu_pitch, previous_imu_pitch);
  }
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  {
    static int skip_iterate = 0;
    if (skip_iterate < process_nth_input_)
    {
      skip_iterate++;
      return;
    }
    ...
    if(min_range_ != MEASUREMENT_MIN_RANGE || max_range_!= MEASUREMENT_MAX_RANGE)
    {
      filtered_scan = removePointsByRange(filtered_scan, min_range_, max_range_);
    }

    pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_scan_ptr(new pcl::PointCloud<pcl::PointXYZ>(filtered_scan));

    // Apply voxelgrid filter
    if (enable_filtering_ && input_voxel_filter_ >= 0.1)
    {
      pcl::VoxelGrid <pcl::PointXYZ> voxel_grid_filter;
      voxel_grid_filter.setLeafSize(input_voxel_filter_, input_voxel_filter_, input_voxel_filter_);
    ...
    */
    ...
    current_pose_imu_odom.roll = 0.0;
    current_pose_imu_odom.pitch = 0.0;
    ...
    */
    ...
    //    br.sendTransform(tf::StampedTransform(transform, current_scan_time, "map", _output_frame_id));
    if (_use_local_transform == true)
    {
      br.sendTransform(tf::StampedTransform(local_transform * transform, current_scan_time, "map", _output_frame_id));
    }
    else
    {
      br.sendTransform(tf::StampedTransform(transform, current_scan_time, "map", _output_frame_id));
    ...
    estimate_twist_msg.header.frame_id = _output_frame_id;
    ...
    */
    ...
    // generate diagnostic msg
    diagnostics_array_msg.header.stamp = ros::Time::now();
    diagnostics_array_msg.status.clear();

    diagnostic_msgs::DiagnosticStatus diag_status_msg;
    diag_status_msg.level = diagnostic_msgs::DiagnosticStatus::OK;
    diag_status_msg.message = "OK";
    diag_status_msg.name = "ndt_matching";
    diag_status_msg.hardware_id = "ndt_matching";

    // ndt reliability
    diagnostic_msgs::KeyValue key_value_msg;
    key_value_msg.key = "ndt_reliability";
    key_value_msg.value = std::to_string(ndt_reliability.data);
    diag_status_msg.values.push_back(key_value_msg);

    // execution time
    key_value_msg.key = "execution_time";
    key_value_msg.value = std::to_string(exe_time);
    diag_status_msg.values.push_back(key_value_msg);


    diagnostics_array_msg.status.push_back(diag_status_msg);
    diagnostics_pub.publish(diagnostics_array_msg);
...
int main(int argc, char** argv)
{
...
#ifdef _AVL
    // check IP addresses
    std::string cmd = "for x in `ls /sys/class/net |grep -v lo`; do cat /sys/class/net/$x/address; done";
    std::string output = ssystem(cmd.c_str());

    bool address_valid = false;
    std::vector<std::string> addresses = StringToLines(output);
    for(auto address : addresses) {
        if ( !address.compare("9c:b6:d0:c5:43:89")                     // Markus NB
                || !address.compare("e8:ea:6a:03:2a:9b")               // AVL Kia
                || !address.compare("00:b0:9d:27:35:94")               // ARG
                || !address.compare("00:1b:21:e4:02:76")               // Ford Fusion                
                ) {
            address_valid = true;
        }
    }
  ...
  // Set log file name.
  private_nh.param("output_log_data", _output_log_data, false);
  if(_output_log_data)
  {
    char buffer[80];
  ...
  private_nh.param("method_type", method_type_tmp, 0);
  _method_type = static_cast<MethodType>(method_type_tmp);

  private_nh.param("mod_keep_2D", mod_keep_2D_, false);
  private_nh.param("use_gnss", _use_gnss, 0);
  private_nh.param("queue_size", _queue_size, 1000);
  //private_nh.param("offset", _offset, "linear");
  private_nh.param("get_height", _get_height, false);
  private_nh.param("use_local_transform", _use_local_transform, false);
  private_nh.param("use_imu", _use_imu, false);
  private_nh.param("use_odom", _use_odom, false);
  private_nh.param("imu_upside_down", _imu_upside_down, false);

  private_nh.param("ndt_res", ndt_res, 1.0);
  private_nh.param("max_iter", max_iter, 30);
  private_nh.param("step_size", step_size, 0.1);
  private_nh.param("trans_eps", trans_eps, 0.01);

  ROS_INFO_STREAM("ndt_resvelodyne_pointcloud: " << ndt_res);
  ROS_INFO_STREAM("max_iter: " << max_iter);
  ROS_INFO_STREAM("step_size: " << step_size);
  ROS_INFO_STREAM("trans_eps: " << trans_eps);

  private_nh.param("enable_filtering", enable_filtering_, true);
  private_nh.param("input_voxel_filter", input_voxel_filter_, 0.5);

  private_nh.param("min_range", min_range_, MEASUREMENT_MIN_RANGE);
  private_nh.param("max_range", max_range_, MEASUREMENT_MAX_RANGE);
  private_nh.param("process_nth_input_", process_nth_input_, 0);

  //private_nh.param("imu_topic", _imu_topic, "/imu/data");
  private_nh.param<std::string>("input_frame_id", _input_frame_id, "lidar_link");
  private_nh.param<std::string>("output_frame_id", _output_frame_id, "base_link");


  private_nh.param("tf_x", _tf_x, 0.0);
  private_nh.param("tf_y", _tf_y, 0.0);
  private_nh.param("tf_z", _tf_z, 0.0);
  private_nh.param("tf_roll", _tf_roll, 0.0);
  private_nh.param("tf_pitch", _tf_pitch, 0.0);
  private_nh.param("tf_yaw", _tf_yaw, 0.0);

  bool tf_lookup = true;
  private_nh.param("tf_lookup", tf_lookup, true);

  // lookup tf between input and output frame_id if required"
  if ( tf_lookup ) {
    tf::TransformListener listener;
    tf::StampedTransform transform;
    for ( int i = 0; i < 10; i++) {
      try{
        listener.lookupTransform(_output_frame_id, _input_frame_id,  
    ...
    pose trans_target_pose;
    tf::Matrix3x3 tmp_m(transform.getRotation());
    tmp_m.getRPY(trans_target_pose.roll, trans_target_pose.pitch, trans_target_pose.yaw);
    ...
    _tf_roll = trans_target_pose.roll;
    _tf_pitch = trans_target_pose.pitch;
    _tf_yaw = trans_target_pose.yaw;
  }
  ...
  std::cout << "tf_lookup: " << tf_lookup << std::endl;
  std::cout << "input_frame: " << _input_frame_id << std::endl;
  std::cout << "output_frame: " << _output_frame_id << std::endl;
  ...
  private_nh.param("initial_pose_z", initial_pose.z, 0.0);
  private_nh.param("initial_pose_roll", initial_pose.roll, 0.0);
  private_nh.param("initial_pose_pitch", initial_pose.pitch, 0.0);
  private_nh.param("initial_pose_yaw", initial_pose.yaw, 0.0);

  ROS_INFO_STREAM("mod_keep_2D: " << mod_keep_2D_);
  ROS_INFO_STREAM("initial_pose_x: " << initial_pose.x);
  ROS_INFO_STREAM("initial_pose_y: " << initial_pose.y);
  ROS_INFO_STREAM("initial_pose_z: " << initial_pose.z);
  ROS_INFO_STREAM("initial_pose_roll: " << initial_pose.roll);
  ROS_INFO_STREAM("initial_pose_pitch: " << initial_pose.pitch);
  ROS_INFO_STREAM("initial_pose_yaw: " << initial_pose.yaw);
  ...
  ndt_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("pose", 10);
