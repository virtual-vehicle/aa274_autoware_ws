...
struct Pose {
  ...
  Pose(double x, double y, double z, double roll, double pitch, double yaw)
      : x(x), y(y), z(z), roll(normalizeAngle(roll)), pitch(normalizeAngle(pitch)), yaw(normalizeAngle(yaw)){};
  ...
  Pose operator+(const Pose &rhs_pose) const {
    ...
    tmp_pose.roll = normalizeAngle(roll + rhs_pose.roll);
    tmp_pose.pitch = normalizeAngle(pitch + rhs_pose.pitch);
    tmp_pose.yaw = normalizeAngle(yaw + rhs_pose.yaw);
    return tmp_pose;
  };
  ...
  Pose operator*(const double val) const {
    ...
    tmp_pose.y = y * val;
    tmp_pose.z = z * val;
    tmp_pose.roll = normalizeAngle(roll * val);
    tmp_pose.pitch = normalizeAngle(pitch * val);
    tmp_pose.yaw = normalizeAngle(yaw * val);
    return tmp_pose;
  };
  ...
  friend std::ostream &operator<<(std::ostream &os, const Pose &pose) {
    ...
    /*
  void setyaw(in_yaw) const {
    yaw = normalizeAngle(in_yaw);
  };
  */
...
private:
  double calcDiffForRadian(const double lhs_rad, const double rhs_rad) const {
    ...
    return normalizeAngle(diff_rad);
  }
...
struct Velocity {
  ...
  Velocity(const Pose &previous_pose, const Pose &current_pose,
           double time_diff_sec, const bool mod_keep_2D_=false) {
    if (time_diff_sec == 0) {
      clear();
      return;
    }
  ...
  Velocity(const PoseStamped &previous_pose, const PoseStamped &current_pose, const bool mod_keep_2D_=false) {
