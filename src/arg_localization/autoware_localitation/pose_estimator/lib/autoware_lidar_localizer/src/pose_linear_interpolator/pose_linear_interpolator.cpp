...
PoseStamped PoseLinearInterpolator::getInterpolatePoseStamped(
    const double time_stamp, const bool mod_keep_2D = false) const {
  return interpolatePose(prev_pose_, current_pose_, time_stamp, mod_keep_2D);
}
...
Velocity PoseLinearInterpolator::getVelocity(const bool mod_keep_2D = false) const {
  return Velocity(prev_pose_, current_pose_, mod_keep_2D);
}
