...
: nh_(nh), private_nh_(private_nh), tf2_listener_(tf2_buffer_), map_frame_("map")
{
  ...
  temp_pose_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("initialpose", 10);
  ...
  autoware_localization_srvs::PoseWithCovarianceStamped::Response & res)
{
  ...
  aligned_pose_msg_ptr->pose.pose.position.x = aligned_pose_msg_ptr->pose.pose.position.x - 535183.7;
  aligned_pose_msg_ptr->pose.pose.position.y = aligned_pose_msg_ptr->pose.pose.position.y - 5211883.0;

  if (succeeded_align) {
    //initial_pose_pub_.publish(*aligned_pose_msg_ptr);
    temp_pose_pub_.publish(*aligned_pose_msg_ptr);
    return true;
  } else {
    return false;
  }
